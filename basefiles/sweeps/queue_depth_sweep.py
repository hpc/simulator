...
def queueDepthSweep(SweepInput,ourInput,origInput):
    ...
    if type(myRange) == bool:
        ...
        if not step:
            ...
            print("Error, config file: queueDepth-sweep but no step")
            sys.exit(1) 
    ...
    if currentExperiments == 0: 
        count = 1
        for i in sweepRange:
            ourInput["experiment_{count}".format(count=count)]={"queue-depth":i}
            count+=1
