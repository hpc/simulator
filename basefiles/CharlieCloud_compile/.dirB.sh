...
# "pd" - Push a bookmark but without output
function pd() {
  # if no argument given, then just pushd and print out the directory stack
  # if $1 is a dash, then just do a "popd" and print out the directory stack
  if [ -z "$1" ]; then
    ...
    if [ -f ~/.DirB/"$1" -a "${1:0:1}" != "." ]; then
      ...
      tmp_amount=`echo $1 | sed 's/+//g' | sed 's/-//g'`
      tmp_search=`echo $1 | sed -E 's/([+-]).*/\1/g'`
      ...
      pushd "$tmp_search$tmp_amount" > /dev/null
    fi
  fi
}
alias pl="dirs -p | nl -w 2 -s '  ' | sed -E 's/([ ]*)([0-9]+.*)/\1-\2/g'"
